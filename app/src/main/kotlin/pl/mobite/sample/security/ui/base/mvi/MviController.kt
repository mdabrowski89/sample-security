...
class MviController<A: MviAction, R: MviResult, VS: MviViewState<R>>(
    ...
    private val updateViewStateBeforeSave: ((VS?) -> VS?)
): LifecycleObserver {
    ...
    fun initViewStatesObservable(savedInstanceState: Bundle?, alwaysInitWithSavedViewState: Boolean) {
        val lastViewState = savedInstanceState?.getParcelable(viewStateParcelKey) as? VS? ?: viewState
        viewModel.initViewStatesObservable(lastViewState, alwaysInitWithSavedViewState)
    }
    ...
    fun saveLastViewState(outState: Bundle) {
        outState.putParcelable(viewStateParcelKey, updateViewStateBeforeSave(viewState))
    }
